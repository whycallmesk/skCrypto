    ...
    <main className="min-h-screen bg-gradient-to-r from-gray-900 to-black p-6 overflow-x-hidden text-white">
      ...
      <div className="container mx-auto px-6">
        <div className="text-center mb-12 space-y-6">
          ...
          <SearchBar searchTerm={searchTerm} onSearch={setSearchTerm} onSelect={handleSearchSelection} />
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {filteredData.map((coin) => (
              <CryptoCard 
                key={coin.id} 
                coin={coin} 
                priceColor={coin.price_change_percentage_24h >= 0 ? "text-green-300" : "text-red-400"} 
              />
            ))}
          </div>
        )}
